...
func GetIpAuditRows(month string, accounts []IpAuditAccount) ([]IpAuditRow, error) {
	...
	prevAggs := make(map[tokenIPKey]ipAuditAggregate, len(prevAggregates))
	prevIPs := make(map[string]int64, len(prevAggregates))
	for _, agg := range prevAggregates {
		prevAggs[tokenIPKey{agg.UserId, agg.TokenName, agg.Ip}] = agg
		key := ipAuditStatusKey(agg.UserId, agg.Ip)
		firstSeen, exists := prevIPs[key]
		if !exists || agg.FirstSeen < firstSeen {
			prevIPs[key] = agg.FirstSeen
		}
	}
	...
	for _, agg := range curAggregates {
		...
		prev := prevAggs[tokenIPKey{agg.UserId, agg.TokenName, agg.Ip}]
		prevFirstSeen, prevSeen := prevIPs[ipAuditStatusKey(agg.UserId, agg.Ip)]
		...
		if prevSeen && prevFirstSeen > 0 && (firstSeen == 0 || prevFirstSeen < firstSeen) {
