...
func AIHubSSOEntry(c *gin.Context) {
	...
	if err != nil {
		if model.IsAIHubUserNotFound(err) {
			...
			user, err = model.CreateAIHubSSOUser(verification.Data.EmployNo, userInfo.UserName, cfg.InitialBalanceRMB, cfg.DefaultGroup, cfg.UserMatchField)
			if err != nil {
...
func setupAIHubSSOSession(c *gin.Context, user *model.User) error {
	...
	model.RecordLoginLog(user.Id, user.Role, user.Username, "Logged in successfully via AI Hub SSO", c.ClientIP(), "login", map[string]any{
		"method": aiHubSSOLoginMethod,
	}, model.AuditOther{
		LoginMethod: aiHubSSOLoginMethod,
