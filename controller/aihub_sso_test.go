...
func TestSetupAIHubSSOSessionCreatesCurrentAuthSession(t *testing.T) {
	...
	var loginAudit model.AuditLog
	require.NoError(t, db.Where("user_id = ? AND category = ?", user.Id, model.AuditCategoryLogin).First(&loginAudit).Error)
	assert.Equal(t, user.Role, loginAudit.ActorRole)
	assert.Equal(t, aiHubSSOLoginMethod, loginAudit.Other.LoginMethod)
	assert.Equal(t, "aihub-sso-test-agent", loginAudit.Other.UserAgent)
