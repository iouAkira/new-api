...
import { useChannelKeyDisclosure } from '@/features/channels/hooks/use-channel-key-disclosure'
...
const EMPTY_NODES: DeploymentNode[] = []
...
function DeploymentChannelKey(props: { channelId: number }) {
  const { t } = useTranslation()
  const disclosure = useChannelKeyDisclosure(true, props.channelId)

  return (
    <>
      {disclosure.channelKey ? (
        <MaskedValueDisplay
          label={t('Current key')}
          fullValue={disclosure.channelKey}
          maskedValue={maskKey(disclosure.channelKey)}
          copyTooltip={t('Copy')}
        ...
        <Button
          variant='outline'
          size='sm'
          onClick={disclosure.handleRevealKey}
          disabled={
            disclosure.isChannelKeyLoading || disclosure.verification.isActive
          }
        >
          {t('Reveal key')}
        </Button>
  ...
  return (
    <div className='space-y-4'>
      <div className='grid grid-cols-2 gap-3 md:grid-cols-4'>
        {['instances', 'nodes', 'models', 'status'].map((key) => (
...
export function ModelNodes() {
  const { t } = useTranslation()
  const canRevealKey = useAuthStore(
  ...
  const allNodes = nodesQuery.data ?? EMPTY_NODES
  ...
  const byModelRows = useMemo(
    () =>
      [...nodes].sort(
        (a, b) => a.model.localeCompare(b.model) || a.ip.localeCompare(b.ip)
      ),
    [nodes]
  )
    ...
    <SectionPageLayout>
      ...
      <SectionPageLayout.Content>
        <div className='space-y-5'>
          ...
          {!loading && !nodesQuery.isError && nodes.length === 0 && (
            <EmptyState
              icon={MapPin}
              title={t('No deployment nodes found.')}
              description={t(
                    ...
                    return (
                      <Card key={ip} className='flex h-36 flex-col'>
                        ...
                        <CardContent className='flex-1 overflow-hidden pt-0'>
                          <div className='[&::-webkit-scrollbar-thumb]:bg-foreground/40 h-full space-y-1 overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent'>
                            {group.map((n) => (
                              <div
                                key={`${n.model}-${n.ip}-${n.port}-${n.channel_id}`}
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        {canRevealKey && (
                          <TableHead>{t('Current key')}</TableHead>
                        )}
                        <TableHead>{t('Status')}</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {pagedModelRows.map((n) => (
                        <TableRow
                          key={`${n.model}-${n.ip}-${n.port}-${n.channel_id}`}
                        >
                          ...
                          {canRevealKey && (
                            <TableCell>
                              <DeploymentChannelKey channelId={n.channel_id} />
                            </TableCell>
                          )}
                          <TableCell>
